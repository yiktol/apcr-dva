...
# Function to check if master_cross_region.yaml exists in S3
verify_master_template() {
    print_status $BLUE "Verifying master template exists in S3..."
    local template_url="https://$Bucket.s3-$BucketRegion.amazonaws.com/$Prefix/master_cross_region.yaml"
    
    if aws s3 ls "s3://$Bucket/$Prefix/master_cross_region.yaml" --region $BucketRegion >/dev/null 2>&1; then
        print_status $GREEN "Master template verified at: $template_url"
    else
        print_status $RED "ERROR: master_cross_region.yaml not found in S3"
        exit 1
    fi
}
